...
class Popup extends React.Component {
  ...
  handlerUsernameChange = (event) => {
    ...
    userInfo.ticketId = event.target.value;
  ...
  render() {
    ...
    return (
      <div>
        ...
        <input type="text" value={UserInfo.ticketId} onChange={this.handlerUsernameChange} />
        <label>Password</label>
        <input type="password" value={UserInfo.password} onChange={this.handlerPasswordChange} />
        <button onClick={() => this.handleLoginAdd(UserInfo.ticketId, UserInfo.password)}>Add</button>
        <button onClick={() => this.handlerClearState()}>Clear</button>
        <span>{UserInfo.ticketId}</span>
      </div>
    );
  }
}
